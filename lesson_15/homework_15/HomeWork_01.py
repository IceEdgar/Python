...
_FILE_PATH = path.dirname(path.realpath(__file__))+'\\HomeWork_01.log'
FORMAT = '{levelname:<4}: {msg}'

logging.basicConfig(format=FORMAT, style='{', filename=_FILE_PATH, filemode='w', encoding='utf-8', level=logging.INFO)
logger = logging.getLogger(__name__)

FileType = namedtuple('FileType', ['name', 'extension', 'folder_flag', 'parrent_folder'])

parser = argparse.ArgumentParser(description="Работа с содержимым каталога")
parser.add_argument('-p', metavar='Путь к каталогу', type=str, nargs=1, help='Введите путь к каталогу', default=[getcwd()])
