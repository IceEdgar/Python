...
class TestEmployee(unittest.TestCase):
    def setUp(self):
        self.employee = Employee("Ivanov", "Ivan", "Ivanovich", 30, "manager", 50000)

    def test_employee_full_name(self):
        self.assertEqual(self.employee.full_name(), "Ivanov Ivan Ivanovich")

    def test_employee_birthday(self):
        initial_age = self.employee.get_age()
        self.employee.birthday()
        new_age = self.employee.get_age()
        self.assertEqual(new_age, initial_age + 1)

    def test_employee_raise_salary(self):
        initial_salary = self.employee.salary
        self.employee.raise_salary(10)
        new_salary = self.employee.salary
        expected_salary = initial_salary * 1.1
        self.assertEqual(new_salary, expected_salary)

    def test_employee_str(self):
        expected_str = "Ivanov Ivan Ivanovich (Manager)"
        self.assertEqual(str(self.employee), expected_str)

    def test_employee_last_name_title(self):
        # self.assertEqual(self.employee.last_name, "Ivanov")
        self.assertEqual(self.employee.last_name, "ivanov")
