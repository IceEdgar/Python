...
# Тесты:
#
# test_employee_full_name: Тестирование метода full_name(). Создайте объект Employee
# с фамилией "Ivanov", именем "Ivan", отчеством "Ivanovich" и убедитесь, что метод full_name()
...
# отчество "Ivanovich", возраст 30, должность "manager" и зарплата 50000. Убедитесь, что метод __str__()
...
def test_employee_birthday(employee):
    employee.birthday()
    assert employee.get_age() == 31


def test_employee_raise_salary(employee):
    employee.raise_salary(10)
    assert employee.salary == 55000.0


def test_employee_str(employee):
    assert employee.__str__() == "Ivanov Ivan Ivanovich (Manager)"


def test_employee_last_name_title(employee):
